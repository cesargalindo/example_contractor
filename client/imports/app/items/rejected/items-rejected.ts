...
import template from "./items-rejected.html";

@Component({
    selector: 'items-rejected',
    template,
})
export class ItemsRejectedComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.apolloItemsCount2 = this.apollo.watchQuery({
            query: gql`
                query ItemsCount2($owner: String, $status: Int) {
    ...
    getItems(page) {
        ...
        this._ngZone.run(() => { // run inside Angular2 world
            this.apolloItems2 = this.apollo.watchQuery({
                query: gql`
                    query MyItems2($owner: String, $status: Int, $options: String) {
